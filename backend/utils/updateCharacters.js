...
async function updateCharacter(character) {
  try {
    const response = await axios.get(
      `https://raider.io/api/v1/characters/profile?region=eu&realm=${character.realm}&name=${character.name}&fields=gear%2Cmythic_plus_scores_by_season%3Acurrent`
    );
    ...
    console.error(
      `Error updating character ${character.realm}/ ${character.name}:`,
      error
    );
  }
}
