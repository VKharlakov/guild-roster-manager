...
function Raid({ sectionType, rosterMaxAmount, setIsPopup, setPopupInfo }) {
  ...
  React.useEffect(() => {
    ...
    if (guildData) {
      guildRMApi
        .getRaidRosters({ parentId: guildData._id })
        .then((rosters) => {
  ...
  async function handleAddCharacter(data) {
    ...
    try {
      ...
      if (!isLimit) {
        ...
        const newCharacter = await guildRMApi.addRaidCharacter({
          ...character,
