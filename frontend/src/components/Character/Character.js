...
function Character({ parentId, character, handleDeleteCharacter }) {
    ...
    <li className="character">
      <a
        className={`character__link${
          isDeleting ? " character__link_no-hover" : ""
        }`}
        href={`${character.rioProfile}`}
        target="_blank"
        rel="noreferrer"
      >
        <button
          className="character__delete-btn"
          onClick={(event) => onDelete(event)}
        />
        <span
          className={`character__role character__role_type_${character.role}`}
        />
        <img
